...
function get_region($id) {
    ...
    $region = D('Home/Region')->where(array('region_id' => $id))->getField('region_name');
    return $region;
}
/**
 *@ 大楼id获取对应的办公类型房间个数
 *@ $rid 大楼id
 */
function count_office_type ($rid, $oid){
    if (!$rid) {
        return false;
    }
    $type = D('ResourcesType')->where(array('rid' => $rid, 'type' => $oid))->count();
    return $type ? $type : 0;
}
/**
 *@ 登记总工位(6种类型)
 ...
 *@ $status 1正常  2活动中
 */
function count_all_office($rid, $status = 1) {
    if (!$rid) {
        return false;
    }
    $allcount = D('ResourcesType')->where(array('rid' => $rid, 'status' => $status))->count();
    return $allcount ? $allcount : 0;
}
//$path => 图片路径  $inout => 内部外部
function grace_path ($path, $inout) {
    if (!$path) {
        return false;
    }
    $imgurl = '/Uploads/Resource/'.$inout.'/'.$path;
    return $imgurl;
}
/**
 * @ 获取办公大楼的server 设施
...
function get_server($sid = 0) {
    ...
    $server = D('Service')->where(array('id' => $sid))->getField('title');
    return $server;
}
...
function cover ($rid){
    ...
    //外部图片取一张做封面
    $cover = D('ResourcesImages')->where(array('rid' => $rid, 'type' => 1))->find();
    return '/Uploads/Resource/Outside/'.$cover['path'];
}
/**
 * 生成用户名
 */
function rand_username (){
    ...
    $user = D('UcenterMember')->where('username = ' . $num)->find();
    if($user){
    	$num = rand_username();
    }
    return $num; 
