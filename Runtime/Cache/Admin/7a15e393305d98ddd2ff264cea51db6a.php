        ...
        <?php if(is_array($keyList)): $i = 0; $__LIST__ = $keyList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$field): $mod = ($i % 2 );++$i;?><label class="item-label"><?php echo (htmlspecialchars($field["title"])); ?>
            ...
            <div class="controls">
                <?php switch($field["type"]): case "text": ?><input type="text" name="<?php echo ($field["name"]); ?>" value="<?php echo (htmlspecialchars($field["value"])); ?>" class="text input-large"/><?php break;?>
                    ...
                    <?php case "imageUrl": ?><div class="controls">
                            <input type="file" id="upload_picture_<?php echo ($field["name"]); ?>">
                            <input type="hidden" name="<?php echo ($field["name"]); ?>" id="cover_id_<?php echo ($field["name"]); ?>" value="<?php echo ($field['value']); ?>"/>
                            <div class="upload-img-box">
                                <?php if(!empty($field["value"])): ?><div class="upload-pre-item"><img src="/Uploads/Images/<?php echo ($field['value']); ?>"/></div><?php endif; ?>
                            </div>
                        </div>
                        <script type="text/javascript">
                            //上传图片
                            ...
                            $("#upload_picture_<?php echo ($field["name"]); ?>").uploadify({
                                ...
                                "fileObjName"     : "download",
                                "buttonText"      : "上传图片",
                                "uploader"        : "<?php echo U('File/uploadImage',array('session_id'=>session_id()));?>",
                                "width"           : 120,
                                'removeTimeout'	  : 1,
                                'fileTypeExts'	  : '*.jpg; *.png; *.gif;',
                                "onUploadSuccess" : uploadPicture<?php echo ($field["name"]); ?>,
                            ...
                            function uploadPicture<?php echo ($field["name"]); ?>(file, data){
                                ...
                                var src = '';
                                if(data.status){
                                    $("#cover_id_<?php echo ($field["name"]); ?>").val( data.savepath + data.savename);
                                    src = data.url || '/Uploads/Images/' + data.savepath + data.savename;
                                    $("#cover_id_<?php echo ($field["name"]); ?>").parent().find('.upload-img-box').html(
                                            '<div class="upload-pre-item"><img src="' + src + '"/></div>'
                                    );
                                } else {
